...
public class Number_03 {
	...
	public static void main(String[] args) {
		...
		// 입력된 문자열에 혹시 있을지 모를 whiteSpace를 제거하고 입력문자열이 QUIT인가를 검사하는 코드
		// 객체의 chaining
		if (strInput.trim().equals("QUIT")) {
			 ...
			 * 	코드를 중단하지 않고, 사용자에게 메시지 등을 전달하고
			 *  적절한 연산코드를 추가하여 코드를 계속 진행하도록 하는 조치
			 *  
			 *  예외처리 코드는
			 *  	개발자에게는 상당히 불필요한 코드를 추가하여 부담을 주는 것일 수 있다
			 *  하지만, 적절한 예외처리를 해준 코드는
			 *  	End User(실제 사용자)에게 불편함을 최소한으로 주는 적절한 방법
			 *  
			 *  End User에게 좋은 코드를 작성하는 결국 좋은 개발자가 된다.
			 */
			try {
