...
/*
 * 인터페이스를 implement 한 클래스는 이름을 지을 때 접미사에 Impl을 붙인다
 * 
 * 인터페이스를 implement 한 클래스는 인터페이스에 설계된 추상메서드를 "반드시" 구체적인 코드로 구현해야 한다.
 * 
		...
		while(true) {
			...
			if (intIn != null) {
				if (intIn < start || intIn > end) {
					System.out.printf("값은 %d ~ %d 까지만 입력하세요\n", start, end);
					continue;
				}
			}
			return intIn;
