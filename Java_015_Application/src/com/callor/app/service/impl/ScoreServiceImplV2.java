...
// V1 을 상속받으면 자동으로 인터페이스도 상속받게 된다
public class ScoreServiceImplV2 extends ScoreServiceImplV1{

	/*
	 * scoreList에 성적정보를 추가할때 학번을 입력받을텐데 이미 등록된 학번 정보가 있으면
	 * 그 학번은 추가하지 못하도록 하는 코드
	 * 
	 * V1의 inputNum() method를 재 정의 한다
	 * V1의 inputNum() method를 재 정의 하기 위하여 private를 protected로 변경했다
	 * 
	 */
	@Override
	protected String inputNum() {
		...
		while(true) {
			...
			// 0 ~ scoreList.size() -1 에 해당한 값을 갖게된다
			for(index = 0 ; index < scoreList.size() ; index++) {
				ScoreVO vo = scoreList.get(index);
				if (vo.getNum().equals(strNum)) {		// vo객체에 학생의 번호가 strNum이 있으면
					break;
				}
			}
