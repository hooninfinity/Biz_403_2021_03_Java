 ...
 * 
 * 객체지향 프로그램언어(java에서는)
 * 	한개의 클래스(파일)내에서 같은 이름의 함수(method)를 중복하여 선언할 수 있다.
...
public class NumberServiceV1 {
	 ...
	 * 
	 * 		단순히 정수면 return 하는 경우는 int type으로 하면 되는데
	 * 		여기에서는 어떤 조건에 따라 null 값을 return 해야 하는 경우도 있기 때문에
...
//		// 두번째 방법
//		// C 언어와 같은 함수형 언어에서는 함수를 끝내는 return문은 한번만 쓰자 라는 전통적인 패턴이 있다
//		// 전통적인 패턴을 지키는 코드의 예
//		Integer sum = null;
//		if ((num1 + num2) % 2 == 0) {
//			sum = num1 + num2;
...
//		return null;
	 ...
	 * 		addNum(float num1, float num2), addNum(double num1, double num2) 형식으로 method를 선언해야 한다
	 * 
