...
public class ScoreServiceImplV1Ex extends ScoreServiceImplV1 {
	...
	public ScoreServiceImplV1Ex() {
		 ...
		 * 	두번째 매개변수는 업무 리스트를 담을 List 형 객체
		 * 
		 * title은 단순 문자열이기 때문에 바로 전달할 수 있는데
		 * List형 객체는 List 객체를 생성하고, 데이터를 add 한 후 전달해야 하므로 약간의 추가 연산 코드가 필요하다
		 * 
...
//		menuService = new MenuServiceImplV1(null, null);
	...
	public void selectMenu() {
		...
		while(true) {
			 ...
			 * 코드가 종료되면
			 * 
			 * 또다시 menu 값이 2인가, menu 값이 3인가를 검사하는 불필요한 코드가 실행된다
			 */
			if (menu == 1) {} // menu 가 1일때
			if (menu == 2) {}
			if (menu == 3) {}
			 ...
			 * menu 가 1일때가 실행되고
			 * 코드가 종료되면
			 * else 이후의 코드는 무시하고 바로 if문이 종료된다
			 * 
