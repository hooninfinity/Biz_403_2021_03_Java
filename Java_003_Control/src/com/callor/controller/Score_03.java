...
public class Score_03 {
	...
	public static void main(String[] args) {
		...
		/*
		 * 과목의 평균을 계산할 때 소수점이하 오류가 발생하여
		 * 소수점이하 계산을 살리기 위해
		 * 평균을 저장할 변수를 float형으로 선언하였다.
		 * 하지만 여전히 계산된 결과는 소수점이하 값이 0으로 나오고 있다.
		 * 
		 * 이 상황은 대입 연산자의 성질 때문.
		 ...
		 * 대입 연산자 오른쪽의 계산식의 값들을 실수형으로 "강제 형변환"한 다음
		 * 연산을 수행해야 한다.
		 */
		...
		// intSum / 2의 연산식에서
		// 정수형변수 intSum을 float형으로 강제 형변환 한 다음
		// 젯수(나누는수) 2도 2.0으로 변환된다.
		// 결국 실수 / 실수 연산이 되어 소수점이하 값을 표현하는 계산 결과가 만들어진다.
		floatAvg = (float)intSum / 2;
		floatAvg = intSum / 2.0F;
		
		floatAvg = intSum / (float)2; // 이건 잘안씀
		floatAvg = (float)intSum / 2.0F; // 이건 잘안씀
		floatAvg = (float)intSum / (float)2; // 이건 잘안씀
