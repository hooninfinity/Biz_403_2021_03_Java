...
//	private Float avg;
	 ...
	 * 총점(total)과 평균(avg)은 점수가 입력되면 항상 계산되어야 하는 항목이다
	 * 즉, 외부에서 이 변수에 값을 저장할일이 별로 없을 것 같다
	 * 		외부에서 total, avg 값을 요청하면 그때그때 계산하여 return을 하면 될 것 같다.
	 * 
	 * 1. 그래서 total, avg 변수를 삭제하고
	 ...
	 * 
	 * 하지만 외부에서는 실제코드가 어떻게 작동되는지 몰라도 또는 관심갖지 않아도 상관없이
