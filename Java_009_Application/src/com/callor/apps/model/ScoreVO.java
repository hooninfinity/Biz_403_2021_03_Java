...
public class ScoreVO {
	...
	/*
	 * 현재 ScoreVO 클래스에는
	 * 7개의 변수와 2개의 method가 설계되어 있는 상태
	 * 이 클래스를 사용하여 만들어진 객체에는
	 * 
	 * 7개의 변수와 2개의 method가 포함된 상태로 완성이 된다.
	 * 
	 * 한번 객체로 생성이 되면, 
	 * 내부의 변수들은 절대 다른 데이터와 혼동되거나 하지 않는다.
	 */
	public int num;
	public String name;
	public int kor;
	 ...
	 * 실제 계산된 결과와 상관없이 임의의 값을 저장할 수 있게 되었다.
	 * 코드가 진행되는 과정에서 total에 임의의 값이 저장되면 평균값이 엉뚱하게 계산되는 결과가 나타난다.
	 * 이런 경우 total, avg 변수를 임의로 수정(저장) 하지 못하도록 막는 장치가 필요하다.
	 * 
	 * VO 클래스의 멤버변수를 private로 선언하면
	 * 다른곳에서 객체를 통해(점연결 연산자를 통해) 직접 접근이 불가능 해 진다.
	 * 
	 * total, avg를 private로 선언하면
	 * 누구도 직접 total, avg에 값을 저장할 수 없다.
	 * 
	 * total, avg 변수에 담긴 값을 객체를 통해 읽게 하기 위해
	 * getTotal(), getAvg() 메서드를 만들어 두었다.
	 * 
	 * 이제부터 total, avg 변수는 저장기능은 되지 않고
